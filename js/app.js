...
function asignarPokerasgos(lvl, stats) {
    ...
    // Modificación: Solo un 0.30% de probabilidad (0.003) de tener Pokérasgos
    if (Math.random() > 0.003) return {};

    let cant = Math.floor(Math.random() * 2) + 1; // Si pasa el filtro, tendrá 1 o 2
...
function generarStats(p, lvl) {
    ...
    // Modificación: Lógica Shiny 0,001% (0.00001)
    let isShiny = Math.random() <= 0.00001; 
    ...
    let jsonVTT = {
        ...
        // Arreglo de bug de la vida exagerada: Enviar solo los puntos extra al Allocated y el maxHP a Health
        "HP": puntosExtra.HP, "ATK": puntosExtra.ATK, "DEF": puntosExtra.DEF, 
        "SPATK": puntosExtra.SPATK, "SPDEF": puntosExtra.SPDEF, "SPEED": puntosExtra.SPEED, 
        "Health": maxHP,
        "Capabilities": capObj,
        ...skillObj,
        ...movs,
...
function getDice(db) {
    let dbNum = parseInt(db);
    if (isNaN(dbNum) || dbNum <= 0) return "0";
    const dbTable = [
        "0", "1d6+1", "1d6+3", "1d6+5", "1d8+6", "1d8+8", "2d6+8", "2d6+10", "2d8+10",
        "2d10+10", "3d8+10", "3d10+10", "3d12+10", "4d10+10", "4d10+15", "4d10+20",
        "5d10+20", "5d12+25", "6d12+25", "6d12+30", "7d12+30"
    ];
    return dbNum > 20 ? "7d12+30 (+ extra)" : dbTable[dbNum];
}
...
function renderResultados(resultados) {
    ...
    resultados.forEach(res => {
        ...
        // --- 1. Generar HTML de Habilidades ---
        let habsHTML = "";
        for(let i=1; i<=3; i++) {
            let ab = res.jsonVTT[`Ability${i}`];
            if(ab && ab.Name) {
                let cat = "Básica";
                if((res.p.system.lists.abilitiesAdvanced || "").includes(ab.Name)) cat = "Avanzada";
                if((res.p.system.lists.abilitiesHigh || "").includes(ab.Name)) cat = "Suprema";
                
                let colorClass = cat === "Básica" ? "bg-success" : cat === "Avanzada" ? "bg-primary" : "bg-danger";
                
                habsHTML += `<li class="mb-2"><span class="badge ${colorClass} me-2">${cat}</span><strong>${ab.Name}</strong> <em>(${ab.Freq})</em>: <span style="color:#ccc;">${ab.Info}</span></li>`;
            }
        }

        // --- 2. Generar HTML de Pokérasgos ---
        let edgesHTML = "";
        for(let i=1; i<=2; i++) {
            let edge = res.jsonVTT[`Edge${i}`];
            if(edge && edge.Name) {
                edgesHTML += `<li class="mb-1"><span class="badge bg-secondary me-2">Pokérasgo</span><strong>${edge.Name}</strong>: <span style="color:#ccc;">${edge.Info}</span></li>`;
            }
        }

        // --- 3. Generar HTML de Movimientos (incluyendo Forcejeo) ---
        let movesHTML = "";
        let movesList = [];
        for(let i=1; i<=6; i++) {
            if(res.jsonVTT[`Move${i}`] && res.jsonVTT[`Move${i}`].Name) {
                movesList.push(res.jsonVTT[`Move${i}`]);
            }
        }
        
        let forcejeoInfo = dbComplementos.movimientos["forcejeo"] || { Effects: "El usuario recibe 1/4 de su Vida Máxima como Daño de Retroceso." };
        movesList.push({
            Name: "Forcejeo (Struggle)",
            Type: res.jsonVTT.Struggle_Type || "Ninguno",
            DType: res.jsonVTT.Struggle_DType || "Físico",
            AC: res.jsonVTT.Struggle_AC || 4,
            DB: res.jsonVTT.Struggle_DB || 4,
            Range: res.jsonVTT.Struggle_Range || "Melé, 1 Objetivo",
            Effects: forcejeoInfo.Effects
        });

        movesList.forEach(m => {
            let dbText = "N/A";
            if(m.DType === "Físico" || m.DType === "Especial") {
                let statVal = m.DType === "Físico" ? res.statsActuales.ATK : res.statsActuales.SPATK;
                dbText = `DB ${m.DB} <strong style="color: #a6e22e;">(${getDice(m.DB)} + ${statVal})</strong>`;
            }
            
            let catColor = m.DType === "Físico" ? "bg-danger" : m.DType === "Especial" ? "bg-info text-dark" : "bg-secondary";

            movesHTML += `
                <div class="move-item border-bottom border-secondary pb-2 mb-2">
                    <div class="d-flex justify-content-between align-items-center mb-1">
                        <strong style="color: #ffcb05; font-size: 1.05em;">${m.Name}</strong>
                        <div>
                            <span class="badge bg-dark border border-light me-1">${m.Type}</span>
                            <span class="badge ${catColor}">${m.DType}</span>
                        </div>
                    </div>
                    <div style="font-size: 0.85em; color: #bbb;" class="mb-1">
                        <strong>AC:</strong> ${m.AC} &nbsp;|&nbsp; <strong>Daño:</strong> ${dbText} &nbsp;|&nbsp; <strong>Rango:</strong> ${m.Range} &nbsp;|&nbsp; <strong>Frecuencia:</strong> ${m.Freq}
                    </div>
        ...
        // Modificación: Agregamos la lógica visual del Shiny
        let especieDisplay = res.isShiny 
            ? `${res.jsonVTT.species} <span title="¡Shiny!" style="color: #ffcb05; text-shadow: 0 0 5px #ffcb05;">⭐</span>` 
            : res.jsonVTT.species;

        // --- 4. Armar la Tarjeta HTML Completa ---
        let tbl = `
            <table class="table table-dark table-bordered mt-2 mb-0">
                ...
                <tr>
                    ...
                    <td>${res.jsonVTT.Nature}</td><td>${res.jsonVTT.maxHP} (${res.statsActuales.HP})</td>
                </tr>
            ...
            ${edgesHTML ? `
            <div class="mt-2">
                <div class="section-title">Pokérasgos</div>
                <ul class="list-unstyled mb-0" style="font-size: 0.9em;">
                    ${edgesHTML}
                </ul>
            </div>` : ''}
